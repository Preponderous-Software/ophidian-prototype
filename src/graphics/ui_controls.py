...
class Slider(UIControl):
    ...
    def draw(self, surface, graphik, config):
        ...
        graphik.drawRectangle(self.x, self.y + self.height // 2 - 2, self.width, 4, track_color)
        
        # Draw slider handle
        handle_x = self.x + int((self.value - self.min_value) / (self.max_value - self.min_value) * self.width) - self.handle_width // 2
        handle_color = config.green if self.focused else config.white
        graphik.drawRectangle(handle_x, self.y, self.handle_width, self.height, handle_color)
...
class Toggle(UIControl):
    ...
    def draw(self, surface, graphik, config):
        ...
        # Draw border first
        graphik.drawRectangle(self.x - 2, self.y - 2, self.width + 4, self.height + 4, border_color)
