...
class OptionsMenu:
    ...
    def initialize_controls(self):
        ...
        self.master_volume_slider = Slider(
            ...
            0,
            200,
            30,  # Position will be set dynamically
            0.0,
            1.0,
        ...
        diff_index = (
            difficulties.index(self.config.difficulty)
            if self.config.difficulty in difficulties
            else 1
        )
        ...
        self.apply_button = Button(
            ...
            0,
            80,
            40,  # Position will be set dynamically
            "Apply",
            self.apply_settings,
        )
    ...
    def update_control_positions(self):
        ...
        if current_width < 800:
            ...
            reduced_spacing = min(
                control_spacing, max(35, (current_height - start_y - 150) // 8)
            )  # 8 controls + buttons
            sound_start_y = start_y
            display_start_y = start_y + reduced_spacing * 4  # After sound controls
            controls_start_y = (
                display_start_y + reduced_spacing * 3
            )  # After display controls
        else:
            # Two column layout for wider screens
            column_gap = max(50, current_width // 16)
            left_column_x = max(
                10, current_width // 2 - control_width - column_gap // 2
            )
            right_column_x = min(
                current_width - control_width - 10, current_width // 2 + column_gap // 2
            )
            sound_start_y = start_y
            display_start_y = start_y
            controls_start_y = start_y + control_spacing * 3
            reduced_spacing = (
                control_spacing  # Use normal spacing for two-column layout
            )
        ...
        if current_width < 800:
            button_y = max(
                self.limit_tick_speed_toggle.y + control_height + 20,
                controls_start_y + spacing_to_use * 2,
            )
        else:
    ...
    def handle_key_down(self, key):
        ...
        if key == pygame.K_ESCAPE:
            ...
            if self.controls:
                ...
                self.current_control_index = (self.current_control_index + 1) % len(
                    self.controls
                )
    ...
    def draw(self):
        ...
        # Draw title
        title_size = max(
            30, min(60, int(self.config.text_size * (current_width / 800)))
        )
        self.graphik.drawText(
            "OPTIONS", current_width // 2, 50, title_size, self.config.green
        )

        # Draw category headers with dynamic positioning
        header_size = max(
            16, min(32, int(self.config.text_size // 3 * (current_width / 800)))
        )
        header_y = 120
        ...
        if current_width < 800:
            ...
            left_header_x = (
                self.master_volume_slider.x + self.master_volume_slider.width // 2
            )
            right_header_x = (
                self.fullscreen_toggle.x + 100
            )  # Offset for better centering
        ...
        instruction_size = max(
            10, min(20, int(self.config.text_size // 4 * (current_width / 800)))
        )
        ...
        if (
            self.feedback_message and current_time - self.feedback_time < 2000
        ):  # Show for 2 seconds
    ...
    def _notify_resolution_change(self):
        ...
        if (
            hasattr(self, "resolution_change_callback")
            and self.resolution_change_callback
        ):
